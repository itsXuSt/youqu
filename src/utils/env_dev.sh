...
for p in ${pip_array[*]}
do
    sudo pip3 install --break-system-packages ${p} > /tmp/env.log 2>&1
    check_status ${p}
...
if [ "${requirements}" != "" ]; then
    ...
    do
        ...
        sudo pip3 install --break-system-packages -r ${requirement}
    done
fi
...
if [ "${webui}" != "" ]; then
    sudo pip3 install --break-system-packages playwright -i ${pypi_mirror}
...
if [ "${remote}" != "" ]; then
    sudo pip3 install --break-system-packages zerorpc -i ${pypi_mirror}
fi

sudo pip3 install --break-system-packages -U auto_uos --extra-index-url ${pypi_mirror} -i http://10.20.54.182:8081 --trusted-host=10.20.54.182 \
> /tmp/env.log 2>&1
if [ $? = 0 ]; then
